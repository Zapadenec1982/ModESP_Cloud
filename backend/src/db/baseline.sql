-- ============================================================
-- baseline.sql — the schema a database in the field started from
-- ============================================================
--
-- A verbatim copy of src/db/schema.sql as of 6fb7c55 (20.03.2026), the oldest
-- version any installed database was created from. NOTHING may be added to this
-- file: it is not a description of the current schema, it is the starting point
-- migrations are applied on top of.
--
-- Its whole job is to make one invariant testable in CI:
--
--     schema.sql applied to an empty database
--   ==  baseline.sql + every migration
--
-- Break that and a fresh install gets a structure production never will, with no
-- error anywhere — which is exactly how idx_events_dedup and four telemetry
-- partition indexes came to be missing in the field while every test passed
-- (migration 049). scripts/check-schema-drift.sh compares the two catalogs.
--
-- So: a structural change belongs in a migration AND in schema.sql. Never here.
-- ============================================================

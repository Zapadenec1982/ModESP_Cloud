...
describe('Users CRUD', () => {
  ...
  it('admin can create user', async () => {
    const res = await request(app)
      .post('/api/users')
      .set(authHeader(admin, tenant.id))
      .send({ email: 'new-user@users.test', password: 'Test1234!Secure', role: 'viewer' });
  ...
  it('viewer cannot create user (403)', async () => {
    const res = await request(app)
      .post('/api/users')
      .set(authHeader(viewer, tenant.id))
      .send({ email: 'fail@users.test', password: 'Test1234!Secure', role: 'viewer' });
  ...
  it('admin cannot escalate to superadmin', async () => {
    const res = await request(app)
      .post('/api/users')
      .set(authHeader(admin, tenant.id))
      .send({ email: 'escalate@users.test', password: 'Test1234!Secure', role: 'superadmin' });
  ...
  it('duplicate email returns conflict', async () => {
    const res = await request(app)
      .post('/api/users')
      .set(authHeader(admin, tenant.id))
      .send({ email: admin.email, password: 'Test1234!Secure', role: 'viewer' });

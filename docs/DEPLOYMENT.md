...
| `modesp-retention-cleanup.timer` | щодня 03:30 | `scripts/cleanup-telemetry.js --apply` (згортає сирі вимірювання за останні `DOWNSAMPLE_LOOKBACK_DAYS` (3) у `telemetry_hourly` і додатково години з `telemetry_dirty_hours` — ті, що контролери догнали з буфера після обриву, будь-якого віку, видаляє сирі рядки старші за `plan_limits.retention_days` організації, інженерні канали (`evap`, `cond`, `setpoint`, `comp`) — за `RAW_ENGINEERING_RETENTION_DAYS` (30), скидає партиції старші за найдовшу ретенцію планів, чистить архів старший за `HOURLY_RETENTION_DAYS` (1095)), потім `cleanup-weather.js --apply` і `cleanup-aux.js --apply` — `weather_observations`, `events`, `notification_log`, неактивні `alarms`, протерміновані `refresh_tokens` | modesp |
...
> **Викочування виправлень аудиту цілісності** (PR #40–#58) має власний порядок дій:
> три міграції, з яких 049 видаляє дублікати й може тривати години, вимкнений на час
> роботи `modesp-retention-cleanup.timer`, необовʼязкове відновлення історії **до**
> міграції і чотири разові запити на пошук уже зіпсованих даних —
> `docs/runbooks/audit-rollout.md`.

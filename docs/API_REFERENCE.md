...
```json
{
  ...
  "comment": "Коментар",
  "manufactured_at": "2024-06-15",
  "haccp_min": -18, "haccp_max": -15, "haccp_product": "заморожені напівфабрикати",
...
документ у формі облікового журналу, який можна показати інспектору.

**Критичні межі.** Кожен рядок журналу порівнюється з критичними межами обладнання:
`devices.haccp_min`/`haccp_max` (задає підприємство, `PATCH /devices/:id`, міграція 046) →
інакше межі тривог самого контролера (`protection.low_limit`/`high_limit` з останнього стану) →
інакше журнал прямо пише «не задано» і відхилення не оцінює. `haccp_product` («що зберігається»)
друкується в шапці обладнання.
...
**Вміст PDF:**
- Заголовок мовою звіту; підприємство (`tenants.legal_name`, код ЄДРПОУ/ІПН), точка (підрозділ)
  з адресою і часовим поясом (`tenant_settings.timezone`, для точки — `sites.timezone`);
  період, періодичність фіксації (= bucket), джерело даних, хто і коли сформував
- Обладнання: назва, ідентифікатор, серійний номер, модель, продукція/призначення, критичні
  межі (з джерелом), періодичність, кількість вимірювань
- Підсумок по каналах (мін/макс/сер., кількість вимірювань) і рядок «Відхилень: N інтервалів
  (год), найдовше, крайнє значення · Прогалин у даних: M інтервалів (год)»
- **Журнал контролю** у місцевому часі точки, згрупований по календарних днях: час,
  температура повітря (сер.), мін/макс інтервалу, інші канали, **відхилення** («Так (> −15 °C)»
  / «Ні» / «Ні (пік поза межами)» / «н/д» без меж), **коригувальні дії / примітка** з тривог
  інтервалу (назва тривоги мовою звіту, значення, підтвердження — час, хто, нотатка, наряд);
  інтервали без вимірювань друкуються як «дані відсутні» (або «— контролер офлайн»); рядки з
  відхиленням виділені; після кожного дня — рядок для ПІБ і підпису відповідальної особи
- **Події та коригувальні дії за період**: тривоги з назвою мовою звіту, важливістю, значенням і
  межею, часом зняття, підтвердженням з нотаткою, повʼязаним нарядом (статус, причина закриття)
  і сервісним записом; окремо — сервісні записи періоду без наряду
- Примітки: датчики й останній сервісний запис; як визначається відхилення; що означають
  прогалини (і що вони потребують ручної відмітки); або що межі не задані
- Блоки «Відповідальна особа» і «Перевірив (відповідальний за HACCP)»: ПІБ / посада / підпис / дата
- Футер на кожній сторінці: код перевірки, SHA-256 даних звіту (охоплює й межі), URL перевірки

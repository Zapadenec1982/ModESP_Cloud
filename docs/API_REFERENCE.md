...
- `model_id` перевіряється так само: модель обладнання належить організації (`device_models.tenant_id`),
  чужа дає **400** `invalid_model`. Інакше енергорозрахунок брав би потужності з чужого обладнання, а
  справжній власник моделі не міг би її видалити — на неї вже посилався б чужий пристрій, і
  `devices_model_id_fkey` не пускав би. Усі три join'и `devices ↔ device_models` (список пристроїв,
  `GET /devices/:id/energy/summary`, кеш потужностей у `mqtt.js`) несуть `m.tenant_id = d.tenant_id`,
  тож застарілий крос-тенантний `model_id` читається як «моделі немає»
...
Видалити сервісний запис. Запис видаляється лише через **той пристрій, якому він належить**:
`:id` розвʼязується як і в сусідніх `GET`/`POST`, і `:recordId` має бути його записом, інакше `404`.
Без цієї привʼязки технік із грантом на один пристрій міг би видалити будь-який сервісний запис
організації, зокрема створені автоматично при закритті наряду.
...
`export.haccp_pdf`, `export.haccp_site_pdf`, `export.service_pdf`, `export.service_site_pdf`) — інспектор чи клієнт завжди може дізнатися, хто і
...
Журнал контролю температурного режиму холодильного обладнання (HACCP) для одного пристрою (PDF) —
документ для менеджера магазину чи інспектора Держпродспоживслужби. Критерій: звіт не породжує
питань, на які немає відповіді всередині нього. Лише температура повітря в зоні продукту; інженерні
канали (випарник, конденсатор) і технічні події сюди не входять.

**Критична межа.** `devices.haccp_max`/`haccp_min` (задає підприємство, `PATCH /devices/:id`,
міграція 046) → інакше межі тривог самого приладу (`protection.high_limit`/`low_limit`) → інакше
«не задано» з підказкою. `haccp_tolerance` (міграція 047) — допустиме відхилення в °C: «не вище −18 °C,
допустиме відхилення 3 °C» означає, що відхилення починається за −15 °C.

**Поріг витримки.** Відхиленням продукту вважається температура за межею (з урахуванням допустимого
відхилення) довше ніж `haccp_excursion_min` хвилин: точка (`sites.haccp_excursion_min`) → організація
(`tenant_settings.haccp_excursion_min`, `PATCH /tenants/:id/settings`) → 30. Інтервали відтайки
(канал `defrost`) з оцінки виключаються і позначаються в журналі.
...
- `channels` приймається для сумісності, але журнал завжди показує лише `air`
...
**Вміст PDF:**
- Шапка: підприємство (`tenants.legal_name`, код ЄДРПОУ/ІПН), точка з адресою і часовим поясом;
  період, періодичність фіксації (= bucket), джерело даних, хто і коли сформував; **метод** («у
  таблиці — середнє за інтервал по первинних вимірюваннях (кожні N хв); мін/макс у підсумку — по всіх
  первинних вимірюваннях за період»; для архіву — відповідне формулювання); **зберігання даних**
  (термін зберігання первинних вимірювань, 3 роки архіву, посилання на доступ за кодом перевірки)
- Обладнання: назва, ідентифікатор, серійний номер, модель, продукція/призначення, **критична межа**
  реченням із джерелом, правило відхилення з порогом витримки
- Підсумок повітря: мін/макс/сер./кількість вимірювань і рядок «Відхилень продукту: N · Розривів запису: M»
- **Журнал контролю** у місцевому часі точки по календарних днях: час, повітря °C (середнє за
  інтервал), межа («≤ −18 (+3)»), відхилення (Так/Ні), примітка («відтайка», «двері > N хв» за
  тривогою дверей з порогом організації, «розрив»); рядки з відхиленням виділені
- **Відхилення продукту**: початок, кінець, тривалість, крайнє значення, коригувальна дія і
  відповідальний (з наряду, створеного в межах відхилення або протягом доби після; інакше порожні
  клітинки для заповнення вручну) або «Відхилень за період не зафіксовано»
- **Розриви запису**: періоди без первинних даних довше ніж один інтервал вимірювання (від початку
  періоду, між вимірюваннями, до кінця) — «дані відсутні з … по …», або «Розривів запису за період не
  зафіксовано». Дані, досинхронізовані з буфера приладу, розривом не є — рядки в базі на місці
- Примітка про повірку з останнім сервісним записом; блоки «Відповідальна особа» і «Перевірив
  (відповідальний за HACCP)»: ПІБ / посада / підпис / дата
- Перевірка автентичності: код, SHA-256 даних звіту (охоплює межі, відхилення й розриви) і **QR-код**
  з URL перевірки; футер на кожній сторінці
...
### `GET /devices/:id/telemetry/service.pdf`
Сервісний звіт обладнання (PDF) — інженерна пара до HACCP-журналу для техніка й сервісної компанії,
з тих самих даних. Журнал відповідає інспектору, цей документ — на питання «що відбувалося з
машиною і чому». Той самий доступ (`checkDeviceAccess`, функція плану `reports`), ті самі параметри
`from`/`to`/`bucket`/`lang`, ті самі заголовки `X-Report-Code`/`X-Report-Sha256`/`X-Report-Source`,
той самий перехід на погодинний архів для старих періодів і та сама перевірка за кодом
(`GET /api/public/report/:code`). У `report_exports` реєструється з `report_type = 'service'`
(архів `GET /reports?type=service`), в аудит — `export.service_pdf`.

**Вміст PDF:**
- Шапка організації/точки і примітка «технічний документ для сервісу; для інспектора — HACCP-журнал»
- Обладнання: назва, ідентифікатор, серійний номер, модель, прошивка і версія протоколу, останні дані,
  продукція, критична межа HACCP з допустимим відхиленням і правило відхилення (поріг витримки)
- **Налаштування приладу (останній стан)** — уставка, гістерезис, межі й затримки тривог, мін. пауза
  і робота компресора, нічний зсув, інтервал/макс. тривалість/завершення відтайки, захисти — з
  `devices.last_state`, підписи мовою звіту, одиниці як у WebUI
- **Температури за період** — усі канали (повітря, випарник, конденсатор, уставка): мін/макс/сер./
  кількість, ΔT повітря − випарник, максимум конденсатора
- **Графік** (векторний, у PDF): повітря, випарник, уставка, лінія критичної межі HACCP; підписи часу
  на круглих місцевих годинах
- **Робота обладнання**: частка часу компресора, пусків (і пусків/год), найдовша робота без зупинки
  (на архіві — лише частка); циклів, загальна і середня тривалість відтайки; тривог дверей і час
  відчинених дверей; періодів і сумарно офлайн (`events` `device_offline`/`device_online`);
  розривів запису і сумарно; відхилень HACCP і сумарно
- **Тривоги за період** — усі, з назвами мовою звіту, важливістю, значенням і межею, часом зняття,
  підтвердженням (хто, коли, нотатка) і нарядом; **Звʼязок із хмарою** — офлайн-періоди
  (незакритий — «досі офлайн»); **Рекомендації з обслуговування** (`maintenance_hints`) — правило,
  важливість, відкрито/закрито, підтвердження, наряд; **Наряди й сервісні записи** за період
- **Інженерний журнал** по інтервалах у місцевому часі: повітря / випарник / конденсатор / уставка,
  компресор %, відтайка, примітки «офлайн / розрив / двері»; рядки відхилення HACCP виділені
- Перевірка автентичності: код, SHA-256 (охоплює ряди, тривоги, офлайн-періоди і статистику
  компресора) і QR-код

**Помилки:** як у `export.pdf`. HACCP-журнал, структура зберігання й CSV-експорти цим звітом не
змінюються.

### `GET /sites/:id/service.pdf`
Сервісний звіт для всіх активних пристроїв точки (до 50) — розділ на пристрій; доступ і параметри
як у `GET /sites/:id/export.pdf`; аудит `export.service_site_pdf`.
...
лише звіти своїх точок і пристроїв. Query: `type=haccp|alarms|energy|service`, `site_id`, `scheduled=true`,
`limit` (≤200), `offset`.
```json
...
`timezone` — назва IANA (`Europe/Kyiv`), перевіряється так само, як `users.timezone`: чужий формат
(`Kyiv`, `UTC+2`) дає **400** `validation_failed`. Колонка потрапляє в `Intl.DateTimeFormat` у кожному
звіті, де є ця точка, а невідома назва кидає там `RangeError` — тобто помилка, прийнята при створенні,
поверталася б 500-ю на HACCP-PDF і зламаним плановим звітом через місяці. Порожнє поле лишається
`null` і заповнюється з відповіді погодного провайдера (теж із перевіркою).
...
Додаткове поле `haccp_excursion_min` (1–1440, `null` — як в організації): поріг витримки відхилення для HACCP-звіту цієї точки.
...
Підписки на сповіщення організації — `/api/notifications` (`GET/POST/DELETE /notifications/subscribers`,
`POST /notifications/test`, `GET /notifications/log`). **Ролі:** admin — це адміністрування, а не
моніторинг: отримувачі тривог організації і журнал доставок з адресами. Раніше маршрути не мали
ролевого гейту, тож переглядач або ключ API зі скоупом `read` міг додати отримувача.
...
{ "timezone": "Europe/Kyiv", "locale": "uk", "electricity_rate": 7.5, "electricity_currency": "UAH",
  ...
  "haccp_excursion_min": 30, "raw_retention_days": 400 }
```

...
Скидання пароля за кодом від адміністратора. Публічний endpoint (без JWT).

**Body:**
```json
{
  "email": "user@example.com",
  "reset_code": "a1b2c3d4e5f67890",
  "new_password": "new_secure_password"
}
```

**Валідація:**
- `email`: обов'язковий, валідний email
- `reset_code`: обов'язковий, рівно 16 символів
- `new_password`: обов'язковий, мінімум 8 символів
...
**Помилки:**
- `400` — невірний код, прострочений код, невірний email
- `429` — rate limit (під `/api/auth`)

**Побічний ефект:** всі refresh токени користувача відкликаються (force re-login).
...
Згенерувати код скидання пароля для користувача. Код дійсний 30 хвилин.

**Ролі:** admin (свій тенант), superadmin (будь-який користувач)
...
Адмін передає код користувачу (email, дзвінок, Telegram). Користувач вводить код на сторінці входу через "Забули пароль?".
...
### `GET /firmware/dl`
Завантаження firmware файлу через підписаний URL (HMAC-SHA256).

**Авторизація:** підпис в query params (без JWT — використовується ESP32).

**Query params:**
- `file` — ім'я файлу
- `expires` — Unix timestamp закінчення
- `sig` — HMAC-SHA256 підпис (32 hex chars)

**Response 200:** Binary `.bin` файл (`application/octet-stream`).

**Помилки:**
- `400` — відсутні параметри або path traversal
- `403` — невірний або прострочений підпис
- `404` — файл не знайдено

> Підписані URL генеруються автоматично при OTA deploy (30-хв термін дії).
...
- 2026-03-15 — Security: GET /firmware/dl (signed URLs for firmware downloads), POST /auth/reset-password (public password reset), POST /users/:id/password-reset (admin code generation).
